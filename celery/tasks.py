...
def check_light_schedules():
    """
    Проверяет расписания включения света и включает лампу, если наступило нужное время.
    """
    ...
    for schedule in active_schedules:
        if current_day in schedule.days:
            from django.devices.views import lamp
            if lamp:
                lamp.turn_on()
                Light.objects.update_or_create(defaults={'is_on': True})
                print(f"Light turned ON according to schedule {schedule.id}")
